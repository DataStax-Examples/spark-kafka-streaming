...
val dseScalaVersionStr = Seq("/bin/sh", "-c", s"ls ${DSE_HOME}/resources/spark/client-lib/scala-compiler*jar").!!.trim
...
val sparkVersionStr = Seq("/bin/sh", "-c", s"ls ${DSE_HOME}/resources/spark/lib/spark-graphx_*jar").!!.trim
val sparkVersionArr = sparkVersionStr.split("-").last.split(".jar")(0).split('.') // expected: spark-core_2.11-2.0.0.1-2cf48f7.jar
val sparkVersion = sparkVersionArr.mkString(".")
...
lazy val streamingDeps = Seq(
  ...
  "org.apache.spark"  %% "spark-mllib-".concat(sparkVersion) % "provided",
